...
void FrameFilter::apply(uint8_t *data, int width, int height, const FilterParams &params)
{
    ...
    if (!qFuzzyIsNull(params.brightness))
        applyBrightness(data, size, params.brightness / 100.0f);
    if (!qFuzzyCompare(params.contrast, 1.0f))
        applyContrast(data, size, params.contrast);
    if (!qFuzzyCompare(params.saturation, 1.0f))
        applySaturation(data, pixels, params.saturation);
    if (!qFuzzyIsNull(params.hue))
        applyHue(data, pixels, params.hue);
    if (params.blur > 0.0f)
    ...
    if (params.grain > 0.0f)
        applyGrain(data, size, params.grain);
    ...
    for (int y = 0; y < h; ++y)
    {
        for (int x = 0; x < w; ++x)
        {
            for (int c = 0; c < 3; ++c)
            {
                ...
                {
                    int gx = -px(y - 1, x - 1, c) + px(y - 1, x + 1, c) - px(y, x - 1, c) + px(y, x + 1, c) - px(y + 1, x - 1, c) + px(y + 1, x + 1, c);
                    int gy = -px(y - 1, x - 1, c) - px(y - 1, x, c) - px(y - 1, x + 1, c) + px(y + 1, x - 1, c) + px(y + 1, x, c) + px(y + 1, x + 1, c);
                    result = std::min(255, (int)std::sqrt(float(gx * gx + gy * gy)));
                }
                else if (type == FilterParams::KernelEmboss)
                {
                    result = std::clamp(
                        -2 * px(y - 1, x - 1, c) - px(y - 1, x, c) - px(y, x - 1, c) + px(y, x, c) + px(y, x + 1, c) + px(y + 1, x, c) + 2 * px(y + 1, x + 1, c) + 128,
                        0, 255);
                }
